...
extern void *app_cache_alloc(size_t size, image_cache_t cache_type);
extern void app_cache_free(void *p);
#endif
...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    gif = app_cache_alloc(sizeof(gd_GIF) + 5 * width * height, IMAGE_CACHE_PSRAM);
#elif LV_COLOR_DEPTH == 16
    gif = app_cache_alloc(sizeof(gd_GIF) + 4 * width * height, IMAGE_CACHE_PSRAM);
#elif LV_COLOR_DEPTH == 8 || LV_COLOR_DEPTH == 1
    gif = app_cache_alloc(sizeof(gd_GIF) + 3 * width * height, IMAGE_CACHE_PSRAM);
#endif
